...
describe('VentaProductoService', () => {
  ...
  let ventaProductoRepo: Partial<Record<keyof Repository<VentaProducto>, jest.Mock>>;
  ...
  beforeEach(async () => {
    ...
    ventaProductoRepo = { create: jest.fn(), save: jest.fn(), find: jest.fn(), findOne: jest.fn(), delete: jest.fn() };
    ...
    service = new VentaProductoService(ventaProductoRepo as any, ventaRepo as any, productoRepo as any);
  });

  // Mocks de entidades para pruebas
  const usuarioMock: Usuario = {
    id_usuario: 1, nombre: 'Usuario Test', correo: 'usuario@test.com', contrasena: '123456', rol: 'empleado',
    ventas: [], facturas: [], updatedAt: new Date(), deletedAt: new Date(),
  };

  const facturacionMock: Facturacion = {
    id_facturacion: 1, fecha_emision: new Date(), total: 100, venta: {} as Ventas, usuario: usuarioMock,
  };

  const categoriaMock: Categoria = {
    id_categoria: 1, nombre: 'Categoria Test', descripcion: 'Descripcion Categoria', productos: [],
  };

  const proveedorMock: Partial<Proveedor> = {
    id_proveedor: 1, nombre: 'Proveedor Test', telefono: '3001234567', correo: 'proveedor@test.com',
    productos: [], encryptEmail: jest.fn(), getDecryptedEmail: jest.fn().mockReturnValue('proveedor@test.com'),
  };

  const productoMock: Producto = {
    id_producto: 1, nombre: 'Producto Test', descripcion: 'Descripcion Producto', precio: 100, stock: 10,
    proveedor: proveedorMock as Proveedor, categoria: categoriaMock, ventaProductos: [],
  };

  const ventaMock: Ventas = {
    id_venta: 1, fecha: new Date(), total: 100, usuario: usuarioMock, facturacion: facturacionMock, ventaProductos: [],
  };
